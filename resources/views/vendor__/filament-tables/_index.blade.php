...
<div @if (!$isLoaded) wire:init="loadTable" @endif x-ignore
    ...
    <x-filament-tables::container>
        <div @if (!$hasHeader) x-cloak @endif
            ...
            @elseif (($heading || $description || $headerActions) && !$isReordering)
                <x-filament-tables::header :actions="$isReordering ? [] : $headerActions" :actions-position="$headerActionsPosition" :description="$description" :heading="$heading" />
            @endif
            ...
            @if ($hasFiltersAboveContent)
                <div x-data="{ areFiltersOpen: @js(!$hasFiltersAboveContentCollapsible) }" @class(['fi-ta-filters-above-content-ctn grid px-4 py-4 sm:px-6'])>
                    <x-filament-tables::filters :apply-action="$getFiltersApplyAction()" :form="$getFiltersForm()" x-cloak x-show="areFiltersOpen" />
            ...
            <div @if (!$hasHeaderToolbar) x-cloak @endif
                ...
                <div class="flex shrink-0 items-center gap-x-4">
                    ...
                    @if ($areGroupingSettingsVisible)
                        <x-filament-tables::groups :direction-setting="$isGroupingDirectionSettingHidden" :dropdown-on-desktop="$areGroupingSettingsInDropdownOnDesktop()" :groups="$groups"
                ...
                @if ($isGlobalSearchVisible || $hasFiltersDialog || $hasColumnToggleDropdown)
                    <div class="ms-auto flex items-center gap-x-4">
                        ...
                        @if ($isGlobalSearchVisible)
                            <x-filament-tables::search-field :debounce="$searchDebounce" :on-blur="$isSearchOnBlur" :placeholder="$getSearchPlaceholder()" />
                        @endif
                        ...
                        @if ($hasFiltersDialog || $hasColumnToggleDropdown)
                            @if ($hasFiltersDialog)
                                <x-filament-tables::filters.dialog :active-filters-count="$activeFiltersCount" :apply-action="$getFiltersApplyAction()"
                                    :form="$getFiltersForm()" :layout="$filtersLayout" :max-height="$getFiltersFormMaxHeight()" :trigger-action="$filtersTriggerAction"
                            ...
                            @if ($hasColumnToggleDropdown)
                                <x-filament-tables::column-toggle.dropdown :form="$getColumnToggleForm()" :max-height="$getColumnToggleFormMaxHeight()"
                                    :trigger-action="$toggleColumnsTriggerAction" :width="$getColumnToggleFormWidth()" />
                            @endif
        ...
        @elseif ($isSelectionEnabled && $isLoaded)
            <x-filament-tables::selection.indicator :all-selectable-records-count="$allSelectableRecordsCount" :colspan="$columnsCount" :page="$page"
                :select-current-page-only="$selectsCurrentPageOnly" x-bind:hidden="!selectedRecords.length" x-show="selectedRecords.length" />
        @endif
        ...
        <div @if (!$isReordering && ($pollingInterval = $getPollingInterval())) wire:poll.{{ $pollingInterval }} @endif @class([
            ...
            @if (($content || $hasColumnsLayout) && $records !== null && count($records))
                @if (!$isReordering)
                    ...
                    @if ($isSelectionEnabled || count($sortableColumns))
                        <div class="flex items-center gap-4 gap-x-6 bg-gray-50 px-4 dark:bg-white/5 sm:px-6">
                            @if ($isSelectionEnabled && !$isReordering)
                                <x-filament-tables::selection.checkbox {{-- Make sure the "checked" state gets re-evaluated after a Livewire request: --}}
                ...
                @else
                    <x-filament::grid :default="$contentGrid['default'] ?? 1" :sm="$contentGrid['sm'] ?? null" :md="$contentGrid['md'] ?? null" :lg="$contentGrid['lg'] ?? null"
                        :xl="$contentGrid['xl'] ?? null" :two-xl="$contentGrid['2xl'] ?? null"
                        x-on:end.stop="$wire.reorderTable($event.target.sortable.toArray())" x-sortable
                        :data-sortable-animation-duration="$getReorderAnimationDuration()" @class([
                        ...
                        @foreach ($records as $record)
                            ...
                            @if ($recordGroupTitle !== $previousRecordGroupTitle)
                                @if ($hasSummary && !$isReordering && filled($previousRecordGroupTitle))
                                    <x-filament-tables::table class="col-span-full">
                                        <x-filament-tables::summary.row :columns="$columns" extra-heading-column
                                            ...
                                            ])" :placeholder-columns="false" :query="$group->scopeQuery(
                                                $this->getAllTableSummaryQuery(),
                                                $previousRecord,
                                            )"
                                ...
                                <x-filament-tables::group.header :collapsible="$group->isCollapsible()" :description="$group->getDescription($record, $recordGroupTitle)"
                                    :label="$group->isTitlePrefixedWithLabel() ? $group->getLabel() : null" :title="$recordGroupTitle" @class([
                                        'col-span-full',
                                    ...
                                    @if ($isSelectionEnabled)
                                        <x-slot name="start">
                                            <div class="px-3">
                                                <x-filament-tables::selection.group-checkbox :page="$page"
                                                    :key="$recordGroupKey" :title="$recordGroupTitle" />
                                            </div>
                                        </x-slot>
                                    @endif
                            ...
                            <div @if ($hasCollapsibleColumnsLayout) x-data="{ isCollapsed: @js($collapsibleColumnsLayout->isCollapsed()) }"
                                ...
                                @class([
                                    ...
                                    'hover:bg-gray-50 dark:hover:bg-white/10 dark:hover:ring-white/20' =>
                                        ($recordUrl || $recordAction) && $contentGrid,
                                    'rounded-xl shadow-sm ring-1 ring-gray-950/5 dark:bg-white/5 dark:ring-white/10' => $contentGrid,
                                    ...$getRecordClasses($record),
                                ])
                                x-bind:class="{
                                    'hidden': {{ $group?->isCollapsible() ? 'true' : 'false' }} &&
                                        isGroupCollapsed(
                                            {{ \Illuminate\Support\Js::from($recordGroupTitle) }},
                                        ),
                                ...
                                <div @class([
                                    ...
                                    @elseif ($isSelectionEnabled && $isRecordSelectable($record))
                                        <x-filament-tables::selection.checkbox :label="__('filament-tables::table.fields.bulk_select_record.label', [
                                            'key' => $recordKey,
                                        ])" :value="$recordKey"
                                            x-model="selectedRecords" :data-group="$recordGroupKey"
                                    ...
                                    <div @class([
                                        ...
                                        <div class="flex-1">
                                            @if ($recordUrl)
                                                <a {{ \Filament\Support\generate_href_html($recordUrl, $openRecordUrlInNewTab) }}
                                                    ...
                                                    <x-filament-tables::columns.layout :components="$columnsLayout"
                                                        :record="$record" :record-key="$recordKey" :row-loop="$loop" />
                                                </a>
                                            @elseif ($recordAction)
                                                @php
                                        ...
                                        @if ($recordHasActions)
                                            <x-filament-tables::actions :actions="$actions" :alignment="!$contentGrid ? 'start md:end' : Alignment::Start"
                                                :record="$record" wrap="-sm" :class="$recordActionsClasses" />
                                        @endif
                                    </div>

                                    @if ($isRecordCollapsible)
                                        <x-filament::icon-button color="gray"
                                            icon-alias="tables::columns.collapse-button" icon="heroicon-m-chevron-down"
                                            x-on:click="isCollapsed = ! isCollapsed" class="mx-1 my-2 shrink-0"
                            ...
                            <x-filament-tables::table class="col-span-full">
                                <x-filament-tables::summary.row :columns="$columns" extra-heading-column
                                    :heading="__('filament-tables::table.summary.subheadings.group', [
                                        'group' => $previousRecordGroupTitle,
                                        'label' => $pluralModelLabel,
                                    ])" :placeholder-columns="false" :query="$group->scopeQuery($this->getAllTableSummaryQuery(), $previousRecord)" :selected-state="$groupedSummarySelectedState[$previousRecordGroupKey] ?? []" />
                            </x-filament-tables::table>
                        @endif
                    </x-filament::grid>
                @endif
                ...
                <x-filament-tables::table :reorderable="$isReorderable" :reorder-animation-duration="$getReorderAnimationDuration()">
                    ...
                    <x-slot name="header">
                        ...
                        @else
                            @if (count($actions) && $actionsPosition === ActionsPosition::BeforeCells)
                                ...
                                @else
                                    <th aria-label="{{ trans_choice('filament-tables::table.columns.actions.label', $flatActionsCount) }}"
                        ...
                        @foreach ($columns as $column)
                            ...
                            <x-filament-tables::header-cell :actively-sorted="$getSortColumn() === $column->getName()" :alignment="$column->getAlignment()" :name="$column->getName()"
                                :sortable="$column->isSortable() && !$isReordering" :sort-direction="$getSortDirection()" :wrap="$column->isHeaderWrapped()" :attributes="\Filament\Support\prepare_inherited_attributes(
                        ...
                        <x-filament-tables::row>
                            ...
                            @foreach ($columns as $column)
                                <x-filament-tables::cell @class([
                                    'fi-table-individual-search-cell-' .
                                    str($column->getName())->camel()->kebab(),
                    ...
                    @if ($records !== null && count($records))
                        ...
                        @foreach ($records as $record)
                            ...
                            @if ($recordGroupTitle !== $previousRecordGroupTitle)
                                @if ($hasSummary && !$isReordering && filled($previousRecordGroupTitle))
                                    <x-filament-tables::summary.row :actions="count($actions)" :actions-position="$actionsPosition"
                                        :columns="$columns" :group-column="$group?->getColumn()" :groups-only="$isGroupsOnly" :heading="$isGroupsOnly
                                            ? $previousRecordGroupTitle
                                            : __('filament-tables::table.summary.subheadings.group', [
                                                'group' => $previousRecordGroupTitle,
                                                'label' => $pluralModelLabel,
                                            ])"
                                        :query="$group->scopeQuery(
                                            $this->getAllTableSummaryQuery(),
                                            $previousRecord,
                                        )" :record-checkbox-position="$recordCheckboxPosition" :selected-state="$groupedSummarySelectedState[$previousRecordGroupKey] ?? []"
                                ...
                                @if (!$isGroupsOnly)
                                    <x-filament-tables::row>
                                        ...
                                        <td colspan="{{ $groupHeaderColspan }}" class="p-0">
                                            <x-filament-tables::group.header :collapsible="$group->isCollapsible()" :description="$group->getDescription($record, $recordGroupTitle)"
                                                :label="$group->isTitlePrefixedWithLabel()
                                                    ? $group->getLabel()
                                                    : null" :title="$recordGroupTitle" />
                                        </td>
                            ...
                            @if (!$isGroupsOnly)
                                <x-filament-tables::row :alpine-hidden="($group?->isCollapsible() ? 'true' : 'false') .
                                    ' && isGroupCollapsed(' .
                                    \Illuminate\Support\Js::from($recordGroupTitle) .
                                    ...
                                    :x-sortable-item="$isReordering ? $recordKey : null" @class([
                                    ...
                                    @if ($isSelectionEnabled && $recordCheckboxPosition === RecordCheckboxPosition::BeforeCells && !$isReordering)
                                        <x-filament-tables::selection.cell>
                                            @if ($isRecordSelectable($record))
                                                <x-filament-tables::selection.checkbox :label="__(
                                                    'filament-tables::table.fields.bulk_select_record.label',
                                                    ['key' => $recordKey],
                                                )"
                                    ...
                                    @foreach ($columns as $column)
                                        ...
                                        <x-filament-tables::cell
                                            ...
                                            <x-filament-tables::columns.column :column="$column" :is-click-disabled="$column->isClickDisabled() || $isReordering"
                                                :record="$record" :record-action="$recordAction" :record-key="$recordKey"
                                                :record-url="$recordUrl" :should-open-record-url-in-new-tab="$openRecordUrlInNewTab" />
                                        </x-filament-tables::cell>
                                    @endforeach
                        ...
                        @if (
                            $hasSummary &&
                                ...
                                filled($previousRecordGroupTitle) &&
                                (!$records instanceof \Illuminate\Contracts\Pagination\Paginator || !$records->hasMorePages()))
                            <x-filament-tables::summary.row :actions="count($actions)" :actions-position="$actionsPosition" :columns="$columns"
                                :group-column="$group?->getColumn()" :groups-only="$isGroupsOnly" :heading="$isGroupsOnly
                                    ? $previousRecordGroupTitle
                                    : __('filament-tables::table.summary.subheadings.group', [
                                        'group' => $previousRecordGroupTitle,
                                        'label' => $pluralModelLabel,
                                    ])" :query="$group->scopeQuery($this->getAllTableSummaryQuery(), $previousRecord)"
                                :record-checkbox-position="$recordCheckboxPosition" :selected-state="$groupedSummarySelectedState[$previousRecordGroupKey] ?? []" :selection-enabled="$isSelectionEnabled" />
                        @endif

                        @if ($hasSummary && !$isReordering)
                            <x-filament-tables::summary :actions="count($actions)" :actions-position="$actionsPosition" :columns="$columns"
                                :group-column="$group?->getColumn()" :groups-only="$isGroupsOnly" :plural-model-label="$pluralModelLabel" :record-checkbox-position="$recordCheckboxPosition"
        ...
        @if (
            ...
            <x-filament::pagination :extreme-links="$hasExtremePaginationLinks()" :page-options="$getPaginationPageOptions()" :paginator="$records"
